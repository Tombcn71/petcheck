...
export function FeedbackButton() {
  ...
  return (
    <>
      {/* Witte blur-achtergrond achter het formulier, alleen op mobiel */}
      {open && (
        <div
          className="fixed inset-0 z-40 bg-white/70 backdrop-blur-md sm:hidden"
          onClick={() => setOpen(false)}
        />
      )}

      {/* Mobiel: gecentreerd, groter formulier */}
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:hidden">
          <div className="w-full max-w-sm bg-white shadow-2xl border-2 border-slate-300 rounded-3xl">
            {formHeader}
            {formInhoud}
          </div>
        </div>
      ...
      {/* Desktop: uitschuivend paneel aan de rand */}
      <div className="hidden sm:flex fixed right-0 top-2/3 -translate-y-1/2 z-50 items-center">
        <div
          className={`transition-all duration-300 ease-in-out overflow-hidden ${
            open ? "w-72 opacity-100" : "w-0 opacity-0"
          }`}>
          <div className="w-72 bg-white shadow-xl border-2 border-slate-300 rounded-l-3xl">
            {formHeader}
            {formInhoud}
          </div>
        </div>
      ...
      {/* Mobiel: verticale tab-knop */}
      {!open && (
        <button
          onClick={() => setOpen(true)}
          className="sm:hidden fixed right-0 top-2/3 -translate-y-1/2 z-50 bg-[#4FC3F7] hover:bg-[#0288D1] text-white transition-all rounded-l-xl flex items-center justify-center shrink-0"
          style={{ width: "42px", height: "110px" }}>
          <span
            className="font-black uppercase text-[10px] tracking-widest whitespace-nowrap"
            style={{ transform: "rotate(-90deg)" }}>
